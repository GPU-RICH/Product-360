...
async def process_question(question: str):
    ...
    try:
        answer, follow_up_questions = await chatbot.process_question(question)
        
        st.session_state.chat_memory.add_interaction(question, answer)
        chatbot.logger.log_interaction(question, answer)
        ...
        if (message_count == 2 and not st.session_state.metadata_collection_state['purchase_collected']) or \
           ...
           (message_count == 6 and not st.session_state.metadata_collection_state['location_collected']) or \
           (message_count == 8 and not st.session_state.metadata_collection_state['name_collected']):
